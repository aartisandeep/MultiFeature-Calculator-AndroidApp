...
//        navigationView.setNavigationItemSelectedListener(this);
    ...
    private void currencyTrends(){
        if(!fromText.equals("") && !toText.equals("")) {
            from = fromText.getText().toString().toUpperCase().replaceAll(" ", "");
            to = toText.getText().toString();
            if((from.replaceAll(" ","").equalsIgnoreCase("USD") || from.replaceAll(" ","").replaceAll(" ","").equalsIgnoreCase("INR") || from.replaceAll(" ","").equalsIgnoreCase("EUR") || from.replaceAll(" ","").equalsIgnoreCase("GBP")) && (to.replaceAll(" ","").equalsIgnoreCase("USD") || to.replaceAll(" ","").equalsIgnoreCase("INR") || to.replaceAll(" ","").equalsIgnoreCase("EUR") || to.replaceAll(" ","").equalsIgnoreCase("GBP"))) {
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
                //System.out.println(LocalDate.now().format(formatter));
                Log.d(this.getClass().toString(), "Date" + LocalDate.now().minusYears(1).format(formatter));
                            ...
                            public void response(HttpResponse response) {
                                if (response.code == 200) {
                                    ...
                                    try {
                                        ...
                                        JSONObject toRate = rates.getJSONObject(to.toUpperCase().replaceAll(" ", ""));
                ...
                Toast.makeText(TrendsActivity.this, "Enter Valid Currency " , Toast.LENGTH_SHORT).show();
            }
        }
